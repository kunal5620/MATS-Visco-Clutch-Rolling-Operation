...
def capture_image_in_background():
    save_directory = r"D:/ANAND Project/MATS Visco Clutch Rolling Operation/Final Model/Trial UI/static/images/"
...
def main():
    ...
    if Data_trigger == "True":
        threading.Thread(target=capture_image_in_background).start()
...
def main():
    while True:
        time.sleep(10)  # Wait 10 seconds before each check
        Data_trigger = readBool(db_number, input_offsets["Data_trigger"], 2)
...
def writeBool(db_number, start_offset, bit_offset, value):
    reading1 = plc.db_read(db_number, start_offset, 1)
    snap7.util.set_bool(reading1, 0, bit_offset, value)
    plc.db_write(db_number, start_offset, reading1)
    return None
...
def main():
    time.sleep(10)  # Adding a delay of 3 seconds before executing the readBool function
